...
class Conductor
{
   ...
   */
  public var trackedSongPosition(default, set):Float = 0;

  function set_trackedSongPosition(value:Float):Float
  {
    return trackedSongPosition = value;
  }
   ...
   */
  public var activeBpm(get, never):Float;

  function get_activeBpm():Float
  {
  ...
  function get_beatLengthMs():Float
  {
    ...
    return ((Constants.SECS_PER_MIN / activeBpm) * Constants.MS_PER_SEC) * (4 / timeSignatureDenominator);
  }
  ...
  {
    return SaveData.getData(SaveType.NOTE_OFFSET);
  }
  ...
  {
    return (safeFrames / 60) * Constants.MS_PER_SEC;
  }
  ...
  public function update(?songPos:Float, applyOffsets:Bool = true, forceDispatch:Bool = false):Void
  {
    ...
    {
      this.trackedSongPosition = FlxMath.bound(songPos, Math.min(this.combinedOffset, 0), currentLength);
      this.songPositionDelta += FlxG.elapsed * 1000;
    }
    else
    {
      this.trackedSongPosition = songPos;
    }

    // Set the song position we are at (for purposes of calculating note positions, etc).

    currentTimeChange = timeChanges[0];
    if (this.trackedSongPosition > 0.0)
    {
      for (i in 0...timeChanges.length)
      {
        if (this.trackedSongPosition >= timeChanges[i].timeStamp) currentTimeChange = timeChanges[i];

        if (this.trackedSongPosition < timeChanges[i].timeStamp) break;
      }
    }

    if (currentTimeChange == null && bpmOverride == null && FlxG.sound.music != null)
    {
      log('WARNING: Conductor is broken, timeChanges is empty.');
    }
    else if (currentTimeChange != null && this.trackedSongPosition > 0.0)
    {
      // roundDecimal prevents representing 8 as 7.9999999
      this.currentStepTime = FlxMath.roundDecimal((currentTimeChange.beatTime * Constants.STEPS_PER_BEAT)
        + (this.trackedSongPosition - currentTimeChange.timeStamp) / stepLengthMs, 6);
      this.currentBeatTime = currentStepTime / Constants.STEPS_PER_BEAT;
      this.currentMeasureTime = getTimeInMeasures(this.trackedSongPosition);
    ...
    else
    {
      ...
      this.currentStepTime = FlxMath.roundDecimal((trackedSongPosition / stepLengthMs), 4);
      this.currentBeatTime = currentStepTime / Constants.STEPS_PER_BEAT;
  ...
  {
    return this.trackedSongPosition + this.songPositionDelta;
  }
  ...
  public function getTimeWithDiff(?soundToCheck:FlxSound):Float
  {
    ...
    @:privateAccess
    this.trackedSongPosition = soundToCheck._channel.position;
  ...
  public function mapTimeChanges(songTimeChanges:Array<SongTimeChange>):Void
  {
    ...
    {
      log('WARNING: Conductor mapped no time changes?');
    }

    // Update currentStepTime
    this.update(this.trackedSongPosition, false);
  }

  public function mapTimeChangesFromSong(song:SwagSong):Void
  {
    mapTimeChanges(SongDataUtils.buildTimeChangesFromSong(song));
  }
  ...
  public static function watchQuick(?target:Conductor):Void
  {
    ...
    FlxG.watch.addQuick('trackedSongPosition', target.trackedSongPosition);
    FlxG.watch.addQuick('activeBpm', target.activeBpm);
