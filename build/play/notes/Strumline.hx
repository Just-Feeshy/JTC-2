...
import flixel.FlxG;
import play.notes.NoteVibrationsHandler.NoteStatus;
...
import play.notes.adapter.Preferences;
import play.notes.notekind.NoteKind;
import play.notes.notekind.NoteKindManager;
import util.SortUtil;
import util.GRhythmUtil;
...
{
  /**
   * Default key count for 4-key strumlines. JTC-2 supports a 5-key mode where
   * a SPACE lane sits between DOWN and UP — see the instance-level `keyCount`.
   */
  public static final DEFAULT_KEY_COUNT:Int = 4;

  /**
   * The directions of a 4-key strumline. For 5-key, read the instance-level
   * `DIRECTIONS` field instead — this static remains for compatibility with
   * call sites that didn't know about 5-key (e.g. PlayState helpers, sort callbacks).
   */
  public static final DEFAULT_DIRECTIONS:Array<NoteDirection> = [NoteDirection.LEFT, NoteDirection.DOWN, NoteDirection.UP, NoteDirection.RIGHT];
  ...
  /**
   * Number of lanes on this strumline. `4` for the default, `5` for fifth-key songs.
   * Set at construction time and immutable thereafter.
   */
  public final keyCount:Int;

  /**
   * Lane directions for this strumline, in left-to-right order. Length equals `keyCount`.
   */
  public final DIRECTIONS:Array<NoteDirection>;
  ...
  {
    scrollSpeed = newScrollSpeed ?? (PlayState.SONG != null ? PlayState.SONG.speed : 1.0);
  }
  ...
  // The original Funkin signature `keyCount:Int = KEY_COUNT` doesn't compile here
  // because Haxe requires default arguments to be compile-time constants and
  // `DEFAULT_KEY_COUNT` is a `static final` (only `static inline` would qualify).
  // The literal `4` is equivalent — `DEFAULT_KEY_COUNT` is `4`.
  public function new(noteStyle:NoteStyle, isPlayer:Bool, keyCount:Int = 4, ?scrollSpeed:Float)
  {
    super();

    this.keyCount = keyCount;
    this.DIRECTIONS = NoteDirection.list(keyCount == 5);
    this.isPlayer = isPlayer;
    this.noteStyle = noteStyle;
    this.noteVibrations = new NoteVibrationsHandler(keyCount);

    // JTC-2 layering note: stock Flixel's FlxTypedSpriteGroup has no `zIndex`
    // field (Funkin patches it). We achieve the equivalent draw order by adding
    // groups in ascending z-order — background first, splashes last. The legacy
    // `zIndex = N` assignments and the `sort(SortUtil.byZIndex, ...)` call in
    // `refresh()` are no-ops in JTC-2.
    var backgroundWidth:Float = keyCount * Strumline.NOTE_SPACING + BACKGROUND_PAD * 2;
    #if mobile
    ...
    this.noteHoldCovers = new FlxTypedSpriteGroup<NoteHoldCover>(0, 0, keyCount);
    ...
    // JTC-2 NoteDirection ints are sparse for 4-key: LEFT=0, DOWN=1, UP=3, RIGHT=4
    // (SPACE=2 is reserved for the 5-key SPACE lane). When used as an array
    // index we always need slot 4 to exist for RIGHT, regardless of `keyCount`.
    this.heldKeys = [];
    for (i in 0...5)
    {
      this.heldKeys[i] = [];
  ...
  /**
   * Refresh the strumline. No-op in JTC-2: stock Flixel doesn't track
   * `zIndex` on `FlxBasic`, so we rely on add-order layering instead (see the
   * ctor comment). Kept for API parity with `funkin.play.notes.Strumline`.
   */
  ...
  override function get_width():Float
  {
    ...
    return keyCount * Strumline.NOTE_SPACING * noteSpacingScale * strumlineScale.x;
  }
  ...
  public function updateNotes():Void
  {
    ...
    for (holdNote in holdNotes.members)
    {
      ...
      else if (conductorInUse.songPosition > holdNote.strumTime && holdNote.hitNote && conductorInUse.songPosition >= 0)
      {
        // Hold note is currently being hit, clip it off.
        // JTC-2 guard: ignore `hitNote` while the song hasn't actually started
        // (`songPosition < 0` during pre-roll / dialogue). The legacy bridge can
        // mark trails hit before the music begins, which would otherwise render
        // them anchored at the strumline as a giant full-height bar.
  ...
  public function onBeatHit():Void
  {
    ...
    if (notes.members.length > 1) haxe.ds.ArraySort.sort(notes.members, compareNoteSprites.bind(FlxSort.ASCENDING));

    if (holdNotes.members.length > 1) haxe.ds.ArraySort.sort(holdNotes.members, compareHoldNoteSprites.bind(FlxSort.ASCENDING));
  }
  ...
  public function applyNoteData(data:Array<SongNoteData>):Void
  {
    ...
    haxe.ds.ArraySort.sort(this.noteData, compareNoteData.bind(FlxSort.ASCENDING));
  }
  ...
  public function playNoteSplash(direction:NoteDirection):Void
  {
    ...
    if (splash != null)
    {
      ...
      if (splash.graphic != null) splash.graphic.destroyOnNoUse = false;
    }
  }
  ...
  public function buildNoteSprite(note:SongNoteData):NoteSprite
  {
    ...
    if (noteSprite != null)
    {
      ...
      noteSprite.direction = note.getDirection(keyCount == 5);
      noteSprite.noteData = note;

      noteSprite.x = this.x;
      noteSprite.x += getXPos(DIRECTIONS[note.getDirection(keyCount == 5) % keyCount]);
      noteSprite.x -= (noteSprite.width - Strumline.STRUMLINE_SIZE) / 2; // Center it
      noteSprite.x -= NUDGE;
      noteSprite.y = -9999;

      if (noteSprite.graphic != null) noteSprite.graphic.destroyOnNoUse = false;
  ...
  public function buildHoldNoteSprite(note:SongNoteData):SustainTrail
  {
    ...
    if (holdNoteSprite != null)
    {
      ...
      holdNoteSprite.noteDirection = note.getDirection(keyCount == 5);
      ...
      holdNoteSprite.x += getXPos(DIRECTIONS[note.getDirection(keyCount == 5) % keyCount]);
      holdNoteSprite.x += STRUMLINE_SIZE / 2;
      holdNoteSprite.x -= holdNoteSprite.width / 2;
      holdNoteSprite.y = -9999;

      // JTC-2 patch: legacy code crashed here when an atlas failed to load.
      // SustainTrail.buildFromAtlas leaves `graphic` null on failure; skip the
      // destroyOnNoUse tweak in that case so the strumline can still tick.
      if (holdNoteSprite.graphic != null) holdNoteSprite.graphic.destroyOnNoUse = false;
  ...
  function constructHoldNoteSprite():SustainTrail
  {
    ...
    else
    {
      // The note sprite pool is full and all note splashes are active.
      // We have to create a new note.
      // JTC-2's SustainTrail ctor takes (direction, sustainLength, atlasKey, scrollSpeed, downscroll, deferGraphic).
      // `deferGraphic = true` skips the atlas build; `setupHoldNoteGraphic(noteStyle)` runs later in buildHoldNoteSprite.
      result = new SustainTrail(0, 0, "", 1.0, Preferences.downscroll, true);
