...
local function addStageSprite(name)
    if insertSpriteToStage ~= nil then
        insertSpriteToStage(stageInsertIndex, name)
    elseif insertSpriteToState ~= nil then
        insertSpriteToState(stageInsertIndex, name)
    elseif addSpriteToStage ~= nil then
        addSpriteToStage(name)
    elseif addSpriteToState ~= nil then
        addSpriteToState(name)
    end

    stageInsertIndex = stageInsertIndex + 1
end

local function applyCharacterLayout()
    if spriteExist ~= nil and setSpritePosition ~= nil then
...
local function addLittleHeanchy()
    ...
    addStageSprite("schoolLittleHeanchy")
end
...
local function applyGameplayCameraSetup()
    if setGameplayCameraFocus ~= nil then
...
function school_stage.generatedStage()
    setupStageMetrics()
    ...
    addStageSprite("schoolSmokeFront")
